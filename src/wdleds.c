...
static struct usb_dev_handle *device_init(char *serial)
{
  ...
  if (serial_length > 63) 
  {
    printf("Serial longer than 63 characters\n");
    return NULL;
  }
  
  /* Convert serial to hexadecimal string */
  for (size_t i=0; i<serial_length; ++i)
      sprintf(&hex_serial[i*2],"%x",(int)serial[i]);
    ...
    for (dev = usbubus->devices;dev;dev = dev->next) {
      ...
      if (dev->descriptor.idVendor == VENDOR_ID) {
        ...
        if ((dev->descriptor.idProduct == PRODUCT_ID_PROI)   ||
          ...
          if (usb_get_string_simple(usb_handle,0x3,dev_serial,128)>0) {
              if (strcasecmp(hex_serial,dev_serial)==0)
                return usb_handle;
          }
